...
interface ImagePickerProps {
  ...
  showCard?: boolean;
  defaultValue?: string;
  onImageChange?: (file: File | null) => void; // Измените тип на File | null
}
...
export default function ImagePicker({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0] || null; // Получаем файл или null
    if (file) {
      generateDataUrl(file, (url) => {
        setDataUrl(url);
        if (onImageChange) onImageChange(file); // Передаем файл в колбэк
      });
    } else {
      setDataUrl(null);
      if (onImageChange) onImageChange(null); // Передаем null, если файл не выбран
    }
  };
  return (
    <React.Fragment>
